...
public:
  ...
  //! Translate array of 4 digits into 7 segment codes
  //!
  //! The method accepts an array of 7 numbers between 0 - 44 and converts it to the
  //! codes required to display the number on a 7 segment display.
  //! Numbers between 10-44 are converted to hexadecimal digits (A-Z) and some symbols
  //! Negative numbers are represents empty segment
  //!
  //! @param chars[] An array of 4 numbers between 0 to 44
  ...
  //! Converts a float to int and displays it with showNumberDecEx method
  //!
  //! Dispalays the given argument as a decimal number. The dots between the digits (or colon)
  //! can be individually controlled
  //!
  //! @param num The number to be shown
  //! @param decimalCount Given num digits after decimal point amount. For example number 4.32 
  //! 		 contains 3 digits and 2 of them are behind decimal point. So decimalCount in this 
  //!		 case will be 2. Minimum amount is 0, maximum amount is 4
  //! @param dots Dot enable. The argument is a bit mask, with each bit corresponding to a dot
  //!        between the digits (or colon mark, as implemented by each module). The MSB is the 
  //!        leftmost dot of the digit being update. For example, if pos is 1, the MSB of dots
  //!        will correspond the dot near digit no. 2 from the left. Dots are updated only on
  //!        those digits actually being update (that is, no more than len digits)
  ...
  //!        the number must be between 0 to 99)
  //! @param pos The position least significant digit (0 - leftmost, 3 - rightmost)
  void printFloat(float num, int decimalCount = 0, uint8_t dots = (0x80>>1), bool leading_zero = true, uint8_t length = 4, uint8_t pos = 0);
